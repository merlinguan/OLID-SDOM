function xk = mf2d_ista_lisdomv4(img, psf, paras, recon_dir)
%% test module
% paras.n_iter = 10000;
% paras.lk = 1;
% % image
...
for kk = 1 : n_iter
    ...
    mu = forward( xk, psf, ft, slice);
    grad = gradient(mu, img, psf, ft, size(xk{2}));
    maxLikelihoodY = maximumLikelihood(mu, img);
    for jj = 0:1000
        ltest = lk*1.1^jj;
        xtest = step(xk, grad, ltest);
        new_mu = forward(xtest, psf, ft, slice);
        newMaxlikelihood = maximumLikelihood(new_mu, img);
        quadratic = maxLikelihoodY + quadraticApprox(xtest, xk, grad, ltest);
    ...
    if (mod(kk,100)==1)
        % display
        display3d_2(xk,74,ft,slice,recon_dir,kk);
    end
end
dc = xk{1};
ac = xk{2};
