...
%
cam_pulse_pt = cam_pulse_pt(1:100);
...
for kk = 1 : size( img_info, 1)
    ...
    img_info(kk).ms = ms;
    %% 
    newImgName = [num2str( img_info(kk).imgNum, '%.3d'), '_angle', num2str( round(img_info(kk).ang), '%.3d'), '_t', num2str(minu, '%.3d'), 'min', num2str( sec, '%.2d'), 's', num2str( ms, '%.3d'),  'ms.tif'];
